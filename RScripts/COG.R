...
tryCatch({
  ...
  AllResults = read_tsv(paste0(home, "/WorkDir/", subdir, "/COG_results/", subdir, "-vs_COG.out"), 
                        col_names = 1:12)
  AllResults$X1 <- AllResults$X1 %>% str_replace("\\.1", "")
  
  ##Protein Ids
  AllProteinIds = read_tsv(paste0(home, "/WorkDir/", subdir, "/AllProteinIds-", subdir, ".txt"),
                           col_names = "ID")
  AllProteinIds$ID <- AllProteinIds$ID %>% str_replace("\\.1", "")
  
  ##COG and CDD resources:
  #Summary info about the CD models
  cddid <- as.tibble(read_tsv(paste0(home, "/COG/cddid.tbl"), 
                              col_names = F))
  #Polish tibble
  ...
  cogdef <- as.tibble(read_tsv(paste0(home, "/COG/cog-20.def.tab"), 
                               col_names = F))
  #Polish tibble
  ...
  cogdet <- as.tibble(read_tsv(paste0(home, "/COG/fun-20.tab"), 
                               col_names = F))
  #Polish tibble
  ...
  write_tsv(AllResults, paste0(home, "/WorkDir/", subdir, "/Final_results/COG-", subdir, "-final.tsv"),
            na = "-")

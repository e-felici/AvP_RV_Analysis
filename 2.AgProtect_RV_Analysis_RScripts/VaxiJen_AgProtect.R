...
tryCatch({
  #Read files
      #VaxiJen results
  vaxijen_data <- as_tibble(read_table(paste0(AgProtect, "/VaxiJen_results/VaxiJen3_predictions.csv"),
                             col_names = c("ID", "X2", "X3", "X4", "X5", "AntigenicityResult", "X7", "X8", "X9")))
  
  #Rare aminoacids
  RareAA <- read_tsv(paste0(AgProtect, "/VaxiJen_Results/IDs_RareAminoAcids"), col_names = "ID")
...
write_tsv(Results, paste0(AgProtect,"/Final_results/VaxiJen-", subdir, "-final.tsv"))
